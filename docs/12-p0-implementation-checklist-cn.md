...
| P0-9 | 演示视频脚本 | 2 分钟 Demo + 提交核验清单 | 完成，见 `docs/13-demo-script-cn.md` 和 `docs/14-submission-checklist-cn.md` |
...
4. 部署一个可公开访问的 demo URL。
5. 按 `docs/13-demo-script-cn.md` 录制 2 分钟 demo，并按 `docs/14-submission-checklist-cn.md` 做提交前核验。

理由：双地址 smoke、退款 smoke、MockUSDC smoke、Sponsored MockUSDC smoke、Move 单元测试、Paylink 文件持久化、公开买家页 sponsored 入口、Sponsor 请求持久化、Paylink 回写、后端幂等保护、最小链上同步、demo 脚本和提交核验清单已完成。下一步的硬证据必须来自真实浏览器钱包、扩展后的 Testnet smoke 和可访问部署链接。
