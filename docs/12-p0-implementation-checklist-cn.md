...
3. 做事件索引或最小链上查询同步，让回执来自真实链上对象和交易。
...
4. 验证并记录浏览器钱包签署 sponsored transaction bytes 的兼容性。
5. 准备黑客松提交材料：公开仓库、部署链接、2 分钟 demo、风险边界说明。

理由：双地址 smoke、退款 smoke、MockUSDC smoke、Sponsored MockUSDC smoke、Move 单元测试、Paylink 文件持久化、公开买家页 sponsored 入口、Sponsor 请求持久化、Paylink 回写和后端幂等保护已完成。下一步的硬证据必须来自真实浏览器钱包和扩展后的 Testnet smoke。
