...
- 单 Paylink 幂等控制。
- 公开 `/pay/:id` 页面尚未直接调用真实 sponsored transaction path。
- 事件索引和真实链上回执。
...
- 单 Paylink 重复 Sponsor 拒绝：未实现，需要按 Paylink/action/status 做幂等控制。
...
1. 浏览器钱包签名兼容验证。
2. 公开买家页接入真实 sponsored transaction path。
3. 单 Paylink 重复 Sponsor 幂等拒绝。
...
1. 公开买家页接入真实 sponsored transaction path。
2. 做事件索引或最小链上查询同步，让回执来自真实链上对象和交易。
3. 做单 Paylink 重复 Sponsor 幂等拒绝。
...
1. 公开 `/pay/:id` 页面接入真实 sponsored transaction build/submit 流程。
2. 增加最小事件索引或链上对象同步，让回执页显示真实链上状态。
3. 增加单 Paylink 重复 Sponsor 幂等拒绝。
4. 验证浏览器钱包签署 sponsored transaction bytes 的兼容性。
5. 准备黑客松提交材料：公开仓库、部署链接、2 分钟 demo、风险边界说明。

理由：双地址 smoke、退款 smoke、MockUSDC smoke、Sponsored MockUSDC smoke、Move 单元测试、Paylink 文件持久化、公开买家页基础版、Sponsor 请求持久化和 Paylink 回写已完成。下一步不是继续证明后端能力，而是把真实 sponsored 链上能力接进买家页。
