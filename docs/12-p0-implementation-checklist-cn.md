...
| 优先级 | 任务 | 产出 | 验收 |
|---|---|---|---|
| P0-1 | 持久化 Paylink | SQLite/Postgres 表 | 服务重启后数据不丢 |
| P0-2 | 独立公开支付页 | `/pay/:id` | 买家无需进入卖家 Dashboard |
...
| 表 | 用途 |
|---|---|
| `paylinks` | Paylink 业务快照 |
| `chain_events` | Indexed Sui events |
| `sponsored_transactions` | Sponsor 请求与 Gas 成本 |
...
- 直接打开 `http://127.0.0.1:5174/pay/<id>` 能看到买家支付页。
- 卖家 Dashboard 和买家公开页互不混淆。
- 公开页不能编辑 Paylink。
...
1. 加持久化，至少让 Paylink、Sponsor 请求和链上 digest 不因 API 重启丢失。
2. 做公开 `/pay/:id` 买家页，把卖家 Dashboard 与买家支付页面分开。
3. 把真实 Paylink ID、链上 escrow object 和 sponsored transaction request 绑定起来。
4. 做事件索引或最小链上查询同步，让回执来自真实链上对象和交易。
5. 验证浏览器钱包对 sponsored transaction bytes 的签名兼容性。
6. 做 2 分钟演示视频脚本和提交材料。
7. 后续再评估真实稳定币或官方 Testnet stablecoin；当前参赛闭环先明确标注为 `mUSDC` 测试币。
...
1. 增加持久化和公开 `/pay/:id` 页面，把 Mock Paylink 与链上对象关联起来。
2. 将 Sponsor 请求、真实 Paylink ID、escrow object、fund/deliver/release/refund digest 做幂等绑定。
3. 增加最小事件索引或链上对象同步，让回执页显示真实链上状态。
4. 验证浏览器钱包签署 sponsored transaction bytes 的兼容性。
5. 准备黑客松提交材料：公开仓库、部署链接、2 分钟 demo、风险边界说明。

理由：双地址 smoke、退款 smoke、MockUSDC smoke、Sponsored MockUSDC smoke 和 Move 单元测试已完成。下一步不是继续证明合约，而是把已证明的链上能力接入一个可给评委操作的 Paylink 产品闭环。
