...
| 文档版本 | v1.1 |
| 文档日期 | 2026-06-11 |
| 当前阶段 | Testnet 合约闭环、MockUSDC 与 Sponsored Gas smoke 已验证；Mock Paylink 已有文件持久化和公开 `/pay/:id` 买家页；Sponsor 请求已文件持久化并可回写 Paylink；真实稳定币、浏览器钱包 sponsored 签名兼容和事件索引尚未完成 |
| 产品负责人 | Founder |
...
| 真实 Paylink 与链上对象关联 | Mock Paylink 已持久化，但仍未与真实链上 escrow object 自动绑定 | P0 |
| 链上事件索引 | 未实现 | P0 |
| 公开买家支付链接路由 | 已实现轻量 `/pay/:id` 页面，当前操作仍走 Mock Paylink API | 已完成基础版 |
