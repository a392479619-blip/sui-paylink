...
| 文档版本 | v1.1 |
| 文档日期 | 2026-06-11 |
| 当前阶段 | Testnet 合约闭环、MockUSDC 与 Sponsored Gas smoke 已验证；Paylink 已有文件持久化和公开 `/pay/:id` 买家页；Sponsor 请求已文件持久化并可回写 Paylink；最小按需链上同步已实现；真实稳定币、浏览器钱包 sponsored 签名兼容和完整事件索引尚未完成 |
| 产品负责人 | Founder |
...
| Testnet 发布 | 已完成 | Package `0x0bd14fb2c341415b418a74b74caa1c5f5ec513e69c7a313da533fa56d6e325b7` |
...
| Mock Paylink 创建与状态演示 | 已完成 | Fastify API，Paylink 写入 `.data/paylinks.json` |
| Mock 状态规则对齐 | 已完成 | Mock 释放必须先交付，退款仅限已注资且未交付托管 |
...
| Sponsored Gas 产品化 | Testnet smoke 已验证；Sponsor 请求已持久化，Paylink ID 可绑定并回写 digest/escrow/gas；重复 sponsor 后端幂等拒绝已实现；前端钱包端到端兼容和扩展 smoke 证据尚未完成 | P0 |
| 买卖双方不同账户验证 | 已完成 | `deployments/testnet-two-party-smoke.json` |
| 真实 Paylink 与链上对象关联 | Paylink 可通过 sponsored record 回写 digest/escrow/gas，并可按需同步链上 Escrow object；真实浏览器钱包端到端证据尚未完成 | P0 |
| 链上事件索引 | 最小按需链上同步已实现；后台常驻 indexer 未实现 | P0 |
| 公开买家支付链接路由 | 已实现 `/pay/:id` 页面，已接入 sponsored build/sign/submit 入口；真实 sponsor 钱包浏览器端到端尚未验证 | 已完成基础版 |
...
| `funded` | 退款 | 买家 | `refunded` | 未交付、未释放、未退款 | 保持 `funded` |
| `delivered` | 争议申请 | 买家 | `delivered` | P0 仅记录/说明，不自动退款 | 保持 `delivered` |
...
- 当前创建动作写入本地 JSON store，不创建链上 Invoice。
- 当前 `publicUrl` 指向 `/pay/:id`，已经有独立公开路由页面，但仍未自动发起真实链上 Paylink 注资。
...
在 P0 没有仲裁机制的情况下，让买家在卖家交付前主动取消并取回托管资金。

### 当前合约规则

- 只有买家可以退款。
- 已注资、未交付、未释放、未退款时可以退款。
- 卖家标记交付后，不再允许自动退款；买家只能释放资金或走争议说明路径。

### 产品风险

当前规则不是完整仲裁。卖家提交交付后，买家不能直接退款，但 P0 也没有链上争议裁决，因此不能把产品描述为“公平仲裁托管”。P0 页面必须明确：

> 本原型没有仲裁；交付前可退款，交付后需要释放资金或进入后续争议流程。
...
且不能已经交付。真实 Testnet 退款路径已通过 `testnet-refund-smoke.json`
记录。
...
| POST | `/api/paylinks` | 创建 Paylink | 写入 `.data/paylinks.json` |
| GET | `/api/paylinks` | 获取 Paylink 列表 | 文件持久化数据 |
| GET | `/api/paylinks/:id` | 获取单个 Paylink | 文件持久化数据 |
...
| GET | `/api/sponsored-transactions` | 查询 sponsored transaction 列表 | 文件持久化数据，可按 `paylinkId` 过滤 |
| GET | `/api/sponsored-transactions/:id` | 查询 sponsored transaction | 文件持久化数据 |
| POST | `/api/sponsored-transactions/:id/submit` | 接收用户签名并提交 sponsor 签名 | 已通过 Testnet smoke 验证；成功后可回写 Paylink |
...
3. 建立事件索引与链上回执。
4. 验证浏览器钱包签署 sponsored transaction bytes。
5. 后续替换或补充真实 Testnet 稳定币 Coin Type。

### 第二优先级：让演示可信

1. 实现事件索引与链上回执。
2. 将 Mock Paylink 状态与真实链上 Escrow 对象关联。
3. 验证浏览器钱包签署 sponsored transaction bytes。
4. 补齐真实交易失败后的重试和状态恢复。
